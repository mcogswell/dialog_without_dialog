...
class AWAPoolDataset(Dataset):
    def __init__(self, params, splits, load_vis_info=False):
        ...
        (pools) have different indices for each split. Here's a summary:
            ...
            img_id: AWA image id (custom integer id; see tools/awa/create_awa_meta.py)
        ...
        # AWA meta data
        print('Loading meta-data from', params['awaInfo'])
        with open(params['awaInfo'], 'r') as f:
            self.awa_meta = json.load(f)
        self.all_images = self.awa_meta['images']
        ...
        # extra hard-coded visualization stuff
        if load_vis_info:
    ...
    def _load_vis_info(self, params):
        ...
        print('Dataloader (vis_info) loading vqa json file: ' + inputJson)
